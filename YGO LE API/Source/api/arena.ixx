...
Hook GetDatBytesHook = Hook(0x140761410);
...
private:
	...
	__declspec(noinline) static bool GetDatBytes_(SByte** data, const char* name)
	{
		error_code filesystem_code;

		const char* actual_name;

		if (*(name + 0x18) < 0x10) // normal string
		{
			actual_name = name;
		}
		else
		...
		if (filesystem::exists(filename, filesystem_code))
		{
		...
		{
			return GetDatBytesHook.CallOriginal(&GetDatBytes_)(data, name);
		}
	}
